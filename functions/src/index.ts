...
export const onOrderCancelled = onDocumentUpdated(
  ...
  async (event) => {
    ...
    if (before.status !== "cancelled" && after.status === "cancelled") {
      ...
      logger.info(
        `Processing cancellation notifications for order ${orderId}`,
        { mealType, date, customerId },
      );
      ...
      try {
        ...
        const kitchenIds = allKitchenQuery.docs
          .filter(
            (d) => !after.kitchenId || d.data().kitchenId === after.kitchenId,
          )
          .map((d) => d.id);
        ...
        await batch.commit();
        logger.info(
          `Successfully dispatched cancellation notifications for order ${orderId}`,
        );
      } catch (error) {
        logger.error(
          `Failed to process cancellation notifications for order ${orderId}`,
          error,
        );
