...
describe("BaseRepository", () => {
  ...
  describe("update", () => {
    ...
    it("propagates errors on update", async () => {
      ...
      await expect(repo.update("1", { name: "Fail" })).rejects.toThrow(
        "Failed to update",
      );
    });
  });
  ...
  describe("createConverter", () => {
    it("toFirestore returns the object", () => {
      ...
      expect(converter.toFirestore({ id: "1", name: "A" })).toEqual({
        id: "1",
        name: "A",
      });
    });
    ...
    it("fromFirestore merges id from snapshot", () => {
      ...
      const mockSnapshot = {
        id: "snap-1",
        data: () => ({ name: "A", id: "wrong" }),
      } as any;
      expect(converter.fromFirestore(mockSnapshot, {})).toEqual({
        id: "snap-1",
        name: "A",
      });
    });
