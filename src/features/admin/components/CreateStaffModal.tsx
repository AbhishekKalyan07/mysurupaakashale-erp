    ...
    if (data.role === "delivery_partner") {
      ...
      if (!data.zoneIds || data.zoneIds.length === 0) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["zoneIds"],
          message: "At least one Delivery Zone must be assigned",
        });
      }
    }
  });
        ...
        <div className="overflow-y-auto p-6">
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <select
                {...register("role")}
                className="w-full h-10 px-3 rounded-lg border border-rice-300 focus:ring-2 focus:ring-leaf-600 bg-white capitalize"
              >
                ...
                <div className="space-y-2">
                  ...
                  {zones.length === 0 ? (
                    <div className="text-xs text-danger italic py-2 bg-white rounded-lg px-3 border border-dashed border-danger">
                      No delivery zones exist. You must create a delivery zone
                      before creating a Delivery Partner.
                    </div>
                  ) : (
