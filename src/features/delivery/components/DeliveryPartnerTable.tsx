  ...
  return (
    <div className="space-y-3">
      {orders.map((order, index) => {
        ...
        const customerName =
          order.customerName || customer?.fullName || "Unknown Customer";
      ...
      {allTerminal && sessionStatus !== "completed" && (
        <Card
          elevated
          className="p-6 bg-success-subtle border-success/20 flex flex-col items-center justify-center text-center gap-4 mt-4"
        >
          ...
          <div>
            ...
            <p className="text-sm text-text-muted mt-1">
              Please finalize your session to log the route completion.
            </p>
          </div>
          <Button
            variant="success"
      ...
      {sessionStatus === "completed" && (
        <Card className="p-6 bg-surface-2 text-center border-border mt-4">
          ...
          <p className="text-sm text-text-muted mt-1">
            You have successfully finished today's route.
          </p>
        </Card>
      )}
    </div>
