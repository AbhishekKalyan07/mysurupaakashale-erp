...
describe("dailyProductionRepository", () => {
  ...
  describe("subscribeToState", () => {
    ...
    it("returns default open state if not exists", () => {
      ...
      expect(onNext).toHaveBeenCalledWith(
        expect.objectContaining({ status: "open", id: date }),
      );
    });
  });
  ...
  describe("closeDay", () => {
    it("throws if day is not locked", async () => {
      ...
      await expect(
        dailyProductionRepository.closeDay(date, userId),
      ).rejects.toThrow("Day must be locked");
    });
  ...
  describe("reopenDay", () => {
    it("throws if day is not closed", async () => {
      vi.mocked(getDoc).mockResolvedValueOnce({ exists: () => false } as any); // default is 'open'
      await expect(
        dailyProductionRepository.reopenDay(date, userId, "reason"),
      ).rejects.toThrow("Only closed days");
    });
