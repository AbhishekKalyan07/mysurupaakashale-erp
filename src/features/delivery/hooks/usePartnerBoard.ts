...
import {
  ...
  type DriverSession,
} from "@/shared/services/firestore/dailyDeliveryRepository";
...
export function usePartnerBoard(
  ...
  const orders = useMemo(
    () => allOrders.filter((o) => o.mealType === mealType),
    [allOrders, mealType],
  );
    ...
    mutationFn: async ({
      ...
      if (order) {
        ...
        if (newStatus === "out_for_delivery") {
          notifyOrderOutForDelivery(order.customerId, orderId, mealType).catch(
            console.error,
          );
          ...
          newStatus === "returned_delivery"
        ) {
          notifyDeliveryFailed(order.customerId, orderId, mealType).catch(
            console.error,
          );
        }
      }
  ...
  const completeRouteMutation = useMutation({
    mutationFn: async () => {
      ...
      const activeRouteOrders = freshOrders.filter(
        (o) => !["cancelled", "skipped"].includes(o.status),
      );
      const isAllTerminal =
        activeRouteOrders.length > 0 &&
        activeRouteOrders.every((o) =>
      ...
      if (!isAllTerminal && activeRouteOrders.length > 0) {
        throw new Error(
          "Not all orders are complete. Please finish all deliveries before completing the route.",
        );
      }
