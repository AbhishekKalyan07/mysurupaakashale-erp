    ...
    <DesktopAuthLayout>
      <div className="w-full bg-white p-6 md:p-8 rounded-[32px] border border-[#e8ded2] shadow-[0_8px_30px_rgb(0,0,0,0.06)] animate-in fade-in duration-500">
        <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-3 w-full">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5a4a44] p-1">{showPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button>
              </div>
            ...
            <div>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5a4a44] p-1">{showConfirmPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button>
              </div>
          ...
          <button type="button" onClick={handleGoogleSignUpClick} disabled={googleLoading} className="w-full h-11 rounded-xl border border-[#e8ded2] bg-white hover:bg-stone-50 text-[#2c150c] font-semibold text-[15px] flex items-center justify-center gap-3 shadow-sm transition-all">
