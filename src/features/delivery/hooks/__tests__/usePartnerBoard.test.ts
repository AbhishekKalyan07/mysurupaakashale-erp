...
vi.mock("@/shared/services/firestore/orderRepository", () => ({
  orderRepository: {
    ...
    getById: vi
      .fn()
      .mockResolvedValue({ customerId: "cust-1", mealType: "lunch" }),
  },
}));
...
vi.mock("@/shared/services/business/deliveryService", () => ({
  deliveryService: {
    getDeliverySummary: vi
      .fn()
      .mockReturnValue({ assigned: 5, delivered: 4, failed: 1, returned: 0 }),
  },
}));

vi.mock("firebase/auth", () => ({
  getAuth: vi.fn(() => ({
    currentUser: { uid: "driver-1", displayName: "Driver" },
  })),
}));
  ...
  runTransaction: vi.fn(async (_db, cb) => {
    const txn = {
      get: vi
        ...
        .mockResolvedValue({
          exists: () => true,
          data: () => ({ outForDeliveryAt: null, deliveredAt: null }),
        }),
      update: vi.fn(),
      set: vi.fn(),
    };
...
describe("usePartnerBoard complete route & notifications", () => {
  ...
  const dailyDeliveryUpdateMock = vi.mocked(
    dailyDeliveryRepository.updateDriverSession,
  );
  ...
  it("throws an error if completeRouteMutation is called when not all orders are terminal", async () => {
    ...
    ).rejects.toThrow("Not all orders are complete.");
  });
  ...
  it("sends notifications based on status", async () => {
    ...
    expect(notifyDeliveredMock).toHaveBeenCalledWith(
      "cust-1",
      "ord-1",
      "lunch",
    );
  ...
  it("triggers onSuccess and onError for updateMutation", () => {
    const updateConfig = vi
      .mocked(useMutation)
      .mock.calls.find((c: any) =>
        c[0].mutationFn.toString().includes("runTransaction"),
      );
    if (updateConfig) {
  ...
  it("triggers onSuccess and onError for completeRouteMutation", () => {
    const completeConfig = vi
      .mocked(useMutation)
      .mock.calls.find(
        (c: any) =>
          c[0].mutationFn.toString().includes("completeRouteMutation") ||
          c[0].mutationKey?.includes("completeRoute"),
      );
    if (completeConfig) {
  ...
  it("completes route successfully if allTerminal is true", async () => {
    ...
    const boardWithTerminal = usePartnerBoard(
      "driver-1",
      "2026-08-01",
      "lunch",
    );
