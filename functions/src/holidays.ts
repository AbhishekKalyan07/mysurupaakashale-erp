 ...
 */
export const onHolidayDeclared = onDocumentCreated(
  "holidays/{holidayId}",
  async (event) => {
    ...
    logger.info(
      `Starting holiday reconciliation for ${date} (Holiday: ${name})`,
    );
    ...
    try {
      ...
      if (eligibleOrders.length > 0) {
        ...
        logger.info(
          `Successfully cancelled ${cancelledCount} orders for holiday on ${date}`,
        );
      }
      ...
      if (allUsers.length > 0) {
        ...
        for (const chunk of userChunks) {
          ...
          refs.forEach(({ user, notifId, ref }, idx) => {
            ...
            if (!snap.exists) {
              ...
              if (
                existing.inAppStatus === "read" ||
                existing.inAppStatus === "archived"
              ) {
                batch.update(ref, {
                  title: `Holiday declared: ${name}`,
        ...
        logger.info(
          `Successfully dispatched notifications for holiday on ${date}`,
        );
      }
    } catch (error) {
      logger.error(
        `Error reconciling orders/notifications for holiday on ${date}:`,
        error,
      );
      throw error;
    }
