...
describe("calculatePayroll", () => {
  it("uses configured basicSalary of 30000 instead of 0 fallback", () => {
    ...
    // If it used 0 fallback, deduction would be 0.
  ...
  it("calculates overtime correctly using configured overtimeRate of 150", () => {
    ...
    const attendance: AttendanceRecord[] = Array.from({ length: 21 }, () =>
      createAttendance("present", 8),
    );
  ...
  it("respects a basicSalary of 0 without falling back to 0", () => {
    const profile: EmployeeSalaryProfile = {
      id: "123",
      basicSalary: 0,
      overtimeRate: 150,
    ...
    const attendance: AttendanceRecord[] = Array.from({ length: 22 }, () =>
      createAttendance("present", 8),
    );
    const result = calculatePayroll(profile, attendance, config);
