...
export const queryKeys = {
  leaves: {
    ...
    byStaff: (staffId: string) =>
      [...queryKeys.leaves.base, "staff", staffId] as const,
...
export function useCreateLeaveRequest() {
  ...
  return useMutation({
    mutationFn: async (
      data: Omit<
        LeaveRequest,
        "id" | "status" | "approvedBy" | "createdAt" | "updatedAt"
      >,
    ) => {
        ...
        if (leaveRecord && (status === "approved" || status === "rejected")) {
          ...
          if (status === "approved") {
            ...
            if (!strandedOrders.empty) {
              ...
              await batch.commit();
              toast(
                `Unassigned ${strandedOrders.size} stranded orders. Please reassign them in Delivery Dashboard.`,
                { icon: "⚠️" },
              );
            }
          }
