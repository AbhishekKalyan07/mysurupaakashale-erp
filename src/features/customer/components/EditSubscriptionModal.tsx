...
export function EditSubscriptionModal({
  ...
  const [enabledMeals, setEnabledMeals] = useState<Record<string, boolean>>({
    breakfast: subscription.mealPreferences.some(
      (p) => p.mealType === "breakfast",
    ),
    lunch: subscription.mealPreferences.some((p) => p.mealType === "lunch"),
    dinner: subscription.mealPreferences.some((p) => p.mealType === "dinner"),
  });

  const getOptionId = (type: string) =>
    subscription.mealPreferences.find((p) => p.mealType === type)
      ?.selectedOptionId || "";
  const [lunchOptionId, setLunchOptionId] = useState<string>(
    getOptionId("lunch") ||
      plan.mealSlots?.find((s) => s.mealType === "lunch")?.options?.[0]?.id ||
      "",
  );
  const [dinnerOptionId, setDinnerOptionId] = useState<string>(
    getOptionId("dinner") ||
      plan.mealSlots?.find((s) => s.mealType === "dinner")?.options?.[0]?.id ||
      "",
  );
  ...
  if (plan.pricingMatrix) {
    ...
    const selectedMealsCount =
      Object.values(enabledMeals).filter(Boolean).length;
    ...
    <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
      <Card className="max-w-xl w-full bg-white p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <p className="text-ink-600 text-sm font-sans mb-6">
          Modify your preferences. Changes will affect future deliveries and
          pro-rata billing.
        </p>
              ...
              <div className="pl-8 space-y-3">
                {plan?.mealSlots
                  ...
                  ?.options?.map((option) => (
                    <label
                      key={option.id}
                      className={`block p-3 rounded-xl border-2 cursor-pointer ${lunchOptionId === option.id && enabledMeals.lunch ? "border-emerald-600 bg-emerald-50/20" : "border-rice-300"}`}
                    >
              ...
              <div className="pl-8 space-y-3">
                {plan?.mealSlots
                  ...
                  ?.options?.map((option) => (
                    <label
                      key={option.id}
                      className={`block p-3 rounded-xl border-2 cursor-pointer ${dinnerOptionId === option.id && enabledMeals.dinner ? "border-emerald-600 bg-emerald-50/20" : "border-rice-300"}`}
                    >
                      <div className="flex items-start gap-3">
                        <input
                          type="radio"
