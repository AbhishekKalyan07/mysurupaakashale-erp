...
class OrderService {
  ...
  private async generateMealOrders(
    ...
    if (existingRun && existingRun.status === "success") {
      console.log(
        `[orderService] ${mealType} orders already successfully generated for ${today}.`,
      );
      return 0;
    }
    ...
    try {
          ...
          try {
            ...
            if (!order.deliveryPartnerId) {
              try {
                ...
                await notifMod.notificationService.notifyAdminAlert(
      ...
      console.log(
        `[DEBUG] Meal: ${mealType}, allSubscriptions: ${allSubscriptions.length}, ordersToCreate: ${ordersToCreate.length}`,
      );
      for (let i = 0; i < ordersToCreate.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          for (let j = 0; j < batchOrders.length; j += BATCH_SIZE_NOTIF) {
            ...
            const promises = chunk.flatMap((o) => {
              const p = [
                m.notificationService.notifyOrderGeneratedCustomer(
                  o.customerId!,
                  o.id!,
                  o.mealType || "meal",
                  o.date!,
              ...
              if (o.deliveryPartnerId) {
                p.push(
                  m.notificationService.notifyOrderGeneratedDriver(
                    o.deliveryPartnerId,
                    o.id!,
                    o.mealType || "meal",
                  ),
            ...
            rejections.forEach((r) =>
              console.error(
                "[orderService] Notification failed:",
        ...
        try {
          ...
          await audit.auditRepository.logAction(
            "orders_generated",
            "system",
            "system",
            "System Auto-Generator",
      ...
      const finalStatus =
        ordersFailed > 0 || notificationsFailed > 0
          ? ordersGenerated > 0
            ? "partial"
            : "failed"
          : "success";
  ...
  async syncCustomerActiveOrders(customerId: string): Promise<void> {
    ...
    const activePartners = allPartners.filter(
      (p) => p.isActive,
    ) as import("./types").DeliveryPartnerProfile[];
    const zoneMap = new Map(allZones.map((z) => [z.id, z]));
  ...
  async restoreOrdersForUnskipDay(
    ...
    subscriptionId: string,
    date: string,
    mealTypes: import("./types").MealType[],
