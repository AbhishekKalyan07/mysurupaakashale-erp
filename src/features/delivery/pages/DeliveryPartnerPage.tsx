...
export function DeliveryPartnerPage() {
  ...
  const assignedZones =
    profile?.role === "delivery_partner" && profile.zoneIds?.length > 0
    ...
    return (
      <div className="max-w-2xl mx-auto p-4">
        <EmptyState
          ...
          icon={<AlertCircle className="text-danger" size={40} />}
        />
      </div>
    );
  }
  ...
  const handleAdvance = async (
    orderId: string,
    newStatus: string,
    result?: { reasonCode: string; notes?: string },
  ) => {
    try {
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-ink-500 font-sans mt-1">
          {today} | {orders?.length || 0} assigned orders |{" "}
          {uniqueCustomersToday} unique customers
        </p>
        {profile?.role === "delivery_partner" && (
          <div className="mt-4 p-3.5 bg-primary/10 border border-primary/20 rounded-xl flex items-center gap-3 shadow-sm animate-in fade-in zoom-in-95 duration-300">
            ...
            <div>
              <p className="text-xs text-primary/80 font-bold uppercase tracking-wider mb-0.5">
                Assigned Zones
              </p>
              <p className="text-lg font-display font-bold text-primary-dark">
                {assignedZones}
              </p>
            </div>
          </div>
      ...
      <DeliveryPartnerTable
        ...
        isAdvancingId={
          updateMutation.isPending
            ? updateMutation.variables?.orderId || null
            : null
        }
