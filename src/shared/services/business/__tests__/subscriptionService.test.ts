...
describe("subscriptionService", () => {
  ...
  describe("createSubscription", () => {
    it("throws if missing required fields", async () => {
      await expect(
        subscriptionService.createSubscription(
          ...
          100,
          {} as any,
          [{ mealType: "lunch", selectedOptionId: "opt1" }],
          "2026-08-01",
          "addr1",
          "monthly",
          null,
    ...
    it("creates a subscription with default deposit if settings not found", async () => {
      vi.spyOn(settingsRepository, "getBusinessSettings").mockResolvedValue(
        null as any,
      );
    ...
    it("creates a subscription with configured deposit", async () => {
      vi.spyOn(settingsRepository, "getBusinessSettings").mockResolvedValue({
        pricing: { securityDepositAmount: 500 },
      } as any);
      vi.spyOn(subscriptionRepository, "create").mockResolvedValue("sub1");
  ...
  describe("approveSubscription", () => {
    it("throws if subscription object is invalid", async () => {
      await expect(
        subscriptionService.approveSubscription(null as any),
      ).rejects.toThrow("Valid subscription object is required.");
    });

    it("returns early if already active (idempotency)", async () => {
      vi.spyOn(subscriptionRepository, "updateStatus").mockResolvedValue(
        undefined,
      );
    ...
    it("throws if subscription is cancelled or expired", async () => {
      await expect(
        ...
        "Cannot approve a cancelled subscription — use renew instead.",
      );
    });
    ...
    it("approves subscription and verifies pending payments", async () => {
      ...
      vi.mocked(paymentRepository.getPaymentsPaginated).mockResolvedValue({
        payments: [
          { id: "p1", subscriptionId: "sub1", status: "pending" } as any,
        ],
        lastDoc: null,
      });
  ...
  describe("rejectSubscription", () => {
    it("throws if subscription object is invalid", async () => {
      await expect(
        subscriptionService.rejectSubscription(null as any),
      ).rejects.toThrow("Valid subscription object is required.");
    });
  ...
  describe("pauseSubscription", () => {
    it("throws if subscription object is invalid", async () => {
      await expect(
        subscriptionService.pauseSubscription(null as any),
      ).rejects.toThrow("Valid subscription object is required.");
    });
  ...
  describe("resumeSubscription", () => {
    it("throws if subscription object is invalid", async () => {
      await expect(
        subscriptionService.resumeSubscription(null as any),
      ).rejects.toThrow("Valid subscription object is required.");
    });
    ...
    it("resumes a paused subscription", async () => {
      ...
      expect(subscriptionRepository.update).toHaveBeenCalledWith(
        "sub1",
        expect.objectContaining({ status: "active", pauseStartDate: null }),
      );
    });
  });
});
