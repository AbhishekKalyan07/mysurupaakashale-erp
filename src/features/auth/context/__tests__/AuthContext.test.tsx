...
describe("AuthContext - Strict Firestore Resolution", () => {
  ...
  beforeEach(() => {
    ...
    (userRepository.subscribeToDoc as any).mockImplementation(
      (_uid: string, cb: any, _errCb: any) => {
  ...
  it("4. persists role to localStorage once fetched", () => {
    ...
    expect(getStatus()).toBe("authenticated");
    expect(JSON.parse(localStorage.getItem("auth_cache_user123")!)).toEqual({
      uid: "user123",
      role: "customer",
    });
  });
  ...
  it("6. handles firestore subscription error", () => {
    ...
    act(() => {
      if (firestoreErrorCallback)
        firestoreErrorCallback(new Error("test error"));
    });
  ...
  it("7. handles timeout for profile load", async () => {
    ...
    (signOutUser as any).mockRejectedValueOnce(
      new Error("timeout signout error"),
    );
  ...
  it("10. handles onAuthStateChanged error", () => {
    ...
    act(() => {
      if (authStateErrorCallback)
        authStateErrorCallback(new Error("test auth error"));
    });
