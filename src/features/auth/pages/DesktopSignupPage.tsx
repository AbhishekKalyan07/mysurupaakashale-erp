      ...
      <div className="w-full bg-white/50 p-6 md:p-8 rounded-[32px] backdrop-blur-md border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.06)] animate-in fade-in duration-500">
        ...
        {signupStep === 1 && pendingGoogleUser && (
          <form onSubmit={handlePhoneStepSubmit} className="flex flex-col gap-4 w-full">
            ...
            <button aria-label="Button action" type="submit" className="w-full h-11 rounded-xl bg-[#5c1417] text-white font-semibold text-[17px] shadow-md hover:bg-[#470f12] transition-colors">
              Next Step
            </button>
            <button aria-label="Button action" type="button" onClick={handleCancelGoogleSignup} className="w-full h-10 text-[15px] font-semibold text-[#5a4a44] hover:text-[#5c1417] transition-colors">
              Cancel
            </button>
          </form>
        )}
          ...
          <form onSubmit={handleCompleteGoogleSignup} className="flex flex-col gap-4 w-full">
            ...
            <div className="flex flex-col gap-3">
              <div className="relative">
                ...
                <button aria-label="Button action" type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5a4a44] p-1">
              ...
              <div className="relative">
                ...
                <input type={showConfirmPassword ? 'text' : 'password'} placeholder="Confirm Password" required value={googleConfirmPassword} onChange={(e) => setGoogleConfirmPassword(e.target.value)}
                  className="h-11 w-full rounded-xl border border-[#e8ded2] bg-white pl-12 pr-12 text-[15px] focus:outline-none focus:border-[#5c1417]" />
                <button aria-label="Button action" type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#5a4a44] p-1">
            ...
            <button aria-label="Button action" type="submit" disabled={googleLoading} className="w-full h-11 rounded-xl bg-[#5c1417] text-white font-semibold text-[17px] shadow-md flex justify-center items-center hover:bg-[#470f12] transition-colors">
