...
const mocks = vi.hoisted(() => {
  return {
    ...
    mockDeliveryZoneRepository: {
      list: vi.fn().mockResolvedValue([{ id: "north", name: "North Zone" }]),
    },
    mockMealPlanRepository: {
      list: vi
        .fn()
        .mockResolvedValue([{ id: "standard", name: "Standard Plan" }]),
    },
...
vi.mock("../../functions/src/repositories", () => ({
  ...
  kitchenRepository: {
    ...
    list: vi
      .fn()
      .mockResolvedValue([
        { id: "kitchen-1", name: "Main Kitchen", isDefault: true },
      ]),
  },
...
describe("active daily-order automation", () => {
  it("does not generate a duplicate run that has already succeeded", async () => {
    mockOrderGenerationRunRepository.getById.mockResolvedValue({
      status: "success",
    });

    await expect(
      orderService.generateDailyOrders("2026-07-29"),
    ).resolves.toEqual({
      success: true,
      message:
        "0 new orders generated. (Orders may have already been generated for today)",
      ordersGenerated: 0,
    });
  ...
  it("skips Sundays without querying Firestore collections", async () => {
    await expect(
      orderService.generateDailyOrders("2026-08-02"),
    ).resolves.toEqual({
      success: true,
  ...
  it("creates deterministic orders, respects skips and assigns a zone-matched delivery partner", async () => {
    ...
    mockGetDoc.mockImplementation(
      async (reference: { segments: unknown[] }) => ({
    ...
    await expect(
      orderService.generateDailyOrders("2026-07-29"),
    ).resolves.toMatchObject({ success: true, ordersGenerated: 1 });
    ...
    expect(batch.set).toHaveBeenCalledWith(
      {
        segments: [
          { name: "test-db" },
          "orders",
          "ord_active-subscription_2026-07-29_breakfast",
        ],
      },
      expect.objectContaining({
        id: "ord_active-subscription_2026-07-29_breakfast",
  ...
  it("skips active holidays during daily order generation", async () => {
    const { holidayRepository } =
      await import("../../functions/src/repositories");
  ...
  it("restoreOrdersForUnskipDay regenerates missing orders for an unskipped day", async () => {
    ...
    mockSubscriptionRepository.getById.mockResolvedValue({
      ...
      endDate: "2026-12-31",
      planTier: "standard",
      mealPreferences: [{ mealType: "lunch", selectedOptionId: "lunch-opt" }],
    });
    mockUserRepository.list.mockResolvedValue([
    ...
    await expect(
      orderService.restoreOrdersForUnskipDay(
        "cust-1",
        "sub-1",
        "2026-08-01",
        ["lunch"],
  ...
  it("restoreOrdersForUnskipDay throws on invalid subscription or date", async () => {
    ...
    ).rejects.toThrow("Subscription sub-1 not found");
    ...
    ).rejects.toThrow("Subscription is paused, not active");
  ...
  it("cancelOrdersForSkipDay commits cancelled orders and logs audit", async () => {
    ...
    await orderService.cancelOrdersForSkipDay("sub-1", "cust-1", "2026-09-05", [
      "breakfast",
    ]);
    ...
    expect(auditSpy).toHaveBeenCalledWith(
      ...
      "o1",
      "order",
      { date: "2026-09-05", mealType: "breakfast" },
    );
  });
  ...
  it("appendCancelOrdersToBatch throws if order is kitchen locked", async () => {
    ...
    await expect(
      orderService.appendCancelOrdersToBatch(
        batch,
        "sub-1",
        "cust-1",
        "2026-09-05",
