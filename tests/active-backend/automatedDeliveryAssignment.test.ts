...
import {
  kitchenRepository,
  holidayRepository,
} from "../../functions/src/repositories";
...
describe("Automated Delivery Partner Assignment", () => {
  ...
  const setupMocks = (
    ...
    vi.spyOn(userRepository, "getById").mockImplementation(
      async (id: string) => customers.find((c) => c.id === id) || null,
    );
