...
describe("extractErrorMessage", () => {
  it("extracts known Firebase error codes", () => {
    const cases: Record<string, string> = {
      ...
      unavailable:
        "Service is temporarily offline. Please check your internet connection.",
      internal:
    ...
    for (const [code, expected] of Object.entries(cases)) {
      expect(extractErrorMessage(new FirebaseError(code, "msg"))).toBe(
        expected,
      );
    }
  });
  ...
  it("strips FirebaseError prefix for unknown codes", () => {
    const error = new FirebaseError(
      "unknown",
      "FirebaseError: Some strange error",
    );
  ...
  it("returns default message for non-error objects", () => {
    ...
    expect(extractErrorMessage({ some: "object" }, "Custom fallback")).toBe(
      "Custom fallback",
    );
  });
});
...
describe("withErrorHandling", () => {
  ...
  it("respects showToast: false", async () => {
    ...
    await expect(
      withErrorHandling(promise, { showToast: false }),
    ).rejects.toThrow("Silent failure");
