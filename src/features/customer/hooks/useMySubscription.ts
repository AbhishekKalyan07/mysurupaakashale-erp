  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { orderRepository } =
        await import("@/shared/services/firestore/orderRepository");
...
export function useCustomerOrderHistoryPaginated(customerId?: string) {
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam }) => {
      ...
      const { orderRepository } =
        await import("@/shared/services/firestore/orderRepository");
      return orderRepository.getCustomerOrdersPaginated(
        customerId,
        20,
        pageParam as any,
      );
...
export function useSubscriptionAccruedBill(subscriptionId?: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ orderRepository }, { subscriptionRepository }] =
        await Promise.all([
  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!subscriptionId || !customerId)
        return { daysOrdered: 0, pausedDates: [] };
      const [{ orderRepository }, { subscriptionRepository }] =
        await Promise.all([
          import("@/shared/services/firestore/orderRepository"),
          import("@/shared/services/firestore/subscriptionRepository"),
