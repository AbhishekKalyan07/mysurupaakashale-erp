...
export function BusinessAnalyticsPage() {
  ...
  const handleExportPNG = async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? (err as Error).message : "Export failed",
        { id: toastId },
      );
    }
  };
  ...
  return (
    <div className="space-y-8">
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              className="bg-white text-primary hover:bg-gold/10 hover:text-gold shrink-0"
            >
      ...
      <div
        ref={dashboardRef}
        className="space-y-8 bg-transparent p-1 rounded-xl"
      >
        {/* Drill-down View */}
        {selectedDateStr && (
          <Card className="p-6 bg-primary/5 border-primary/20 shadow-sm relative ring-1 ring-gold/20">
            ...
            <h3 className="text-lg font-display font-bold text-primary mb-2">
              Drill-down: {selectedDateStr}
            </h3>
            <p className="text-sm text-text-muted mb-6">
              You clicked on a specific date. Here are the detailed metrics for
              this day.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              <MetricCard
                title="Daily Revenue"
                value={`₹${data?.revenue.dailyTrend.find((d) => d.date === selectedDateStr)?.amount || 0}`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card hoverLift className="p-6">
            <div className="mb-6">
              ...
              <p className="text-sm text-text-muted">
                Click a data point to drill down to that specific date.
              </p>
            </div>
            <RevenueTrendChart
          ...
          <Card hoverLift className="p-6">
            <div className="mb-6">
              ...
              <p className="text-sm text-text-muted">
                Distribution of orders across breakfast, lunch, and dinner
              </p>
            </div>
          ...
          <Card hoverLift className="p-6">
            <div className="mb-6">
              ...
              <p className="text-sm text-text-muted">
                Current active and paused subscriptions by tier
              </p>
            </div>
            <PlanDistributionChart
          ...
          <Card hoverLift className="p-6">
            <div className="mb-6">
              ...
              <p className="text-sm text-text-muted">
                Total verified collections distributed by method
              </p>
            </div>
          ...
          <Card className="p-6 bg-gradient-to-br from-background to-blue-50/30 border-blue-200/50">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-background rounded-xl shadow-sm border border-blue-100">
                ...
                <p className="text-xl font-bold text-primary font-display">
                  {Object.entries(data?.delivery.byArea || {}).sort(
                    (a, b) => b[1] - a[1],
                  )[0]?.[0] || "N/A"}
                </p>
              </div>
              <div className="p-4 bg-background rounded-xl shadow-sm border border-blue-100">
                ...
                <p
                  className="text-xl font-bold text-primary font-display truncate"
                  title={data?.delivery.topPartner}
                >
                  {data?.delivery.topPartner === "N/A"
                    ? "N/A"
                    : "Partner ID: " + data?.delivery.topPartner.slice(0, 6)}
                </p>
              </div>
            </div>
          </Card>
