...
export function parseAuditLogDetails(details: any): {
  isRawDump: boolean;
  ...
  lines.forEach((line) => {
    ...
    if (trimmed.startsWith("✓")) {
      ...
      trimmed.startsWith("✗") ||
      (trimmed.toLowerCase().includes("fail") && trimmed.includes(">"))
    ) {
      report.tests.push({
      ...
      if (match) {
        ...
        if (file && !report.filesChecked.includes(file)) {
          const cleanFile = file.includes("src/")
            ? file.substring(file.indexOf("src/"))
            : file;
    ...
    if (trimmed.match(/^#+\s+/)) {
      ...
      else if (header.includes("check") || header.includes("performed"))
        currentSection = "checks";
      else if (header.includes("issue") || header.includes("found"))
  ...
  else if (report.status === "unknown" && passedTests > 0)
  ...
  return {
    ...
    cleanDetails:
      typeof details === "string"
        ? { message: "[Parsed Structured Report]" }
        : cleanDetails,
  };
}
