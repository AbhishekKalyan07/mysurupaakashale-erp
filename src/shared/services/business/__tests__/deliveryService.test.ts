...
describe("deliveryService", () => {
  ...
  describe("Aggregation Methods", () => {
    ...
    it("getAreaDeliveryGroups groups by Partner -> Area -> Orders", () => {
      ...
      const groups = deliveryService.getAreaDeliveryGroups(
        orders,
        partnerMap,
        zoneMap,
      );
      ...
      expect(
        driver1.areas.find((a) => a.areaName === "Zone 1")!.orders.length,
      ).toBe(2);

      const unassigned = groups.find(
        (g) => g.partnerName === "Unassigned Partner",
      )!;
  ...
  describe("Lifecycle Methods", () => {
    ...
    describe("assignDriver", () => {
      it("throws if missing args", async () => {
        await expect(
          deliveryService.assignDriver("", "d1", adminId),
        ).rejects.toThrow("Order ID and Driver ID are required.");
      });

      it("throws if order not found", async () => {
        vi.mocked(orderRepository.getById).mockResolvedValue(null);
        await expect(
          deliveryService.assignDriver("o1", "d1", adminId),
    ...
    describe("markPickedUp", () => {
      it("throws if missing args", async () => {
        await expect(
          deliveryService.markPickedUp("", driverId),
        ).rejects.toThrow("Order ID and Driver ID are required.");
      });
      ...
      it("throws if driver mismatch", async () => {
        ...
        await expect(
          deliveryService.markPickedUp("o1", driverId),
        ).rejects.toThrow("Cannot update order assigned to another driver.");
      });
    ...
    describe("startDelivery", () => {
      it("throws if invalid status", async () => {
        ...
        await expect(
          deliveryService.startDelivery("o1", driverId),
        ).rejects.toThrow(
          "Cannot transition from ready_for_pickup to out_for_delivery.",
        );
      });
    ...
    describe("markDelivered", () => {
      it("throws if invalid status", async () => {
        ...
        await expect(
          deliveryService.markDelivered("o1", driverId),
        ).rejects.toThrow("Cannot transition from picked_up to delivered.");
      });
