...
import { getCachedDateTimeFormatter } from "../../../lib/date";
...
describe("Unskip Meal Feature", () => {
  ...
  describe("subscriptionRepository.removeSkip (Cutoff and DB)", () => {
    it("1 & 2. Unskip before cutoff succeeds and removes skip record", async () => {
      const mockGetDoc = await import("firebase/firestore").then(
        (m) => m.getDoc as import("vitest").Mock,
      );
      mockGetDoc.mockResolvedValue({
        exists: () => true,
        data: () => ({ mealTypes: ["breakfast", "lunch"] }),
      });
      const mockSetDoc = await import("firebase/firestore").then(
        (m) => m.setDoc as import("vitest").Mock,
      );
    ...
    it("6. Unskip after cutoff fails", async () => {
      ...
      await expect(
        subscriptionRepository.removeSkip("sub1", today, ["breakfast"], "uid1"),
  ...
  describe("orderService.restoreOrdersForUnskipDay", () => {
    it("3. Existing skipped order is restored to scheduled", async () => {
      ...
      vi.spyOn(orderService, "syncCustomerActiveOrders").mockResolvedValue(
        undefined,
      );

      const mockWriteBatch = await import("firebase/firestore").then(
        (m) => m.writeBatch as import("vitest").Mock,
      );
      ...
      expect(orderService.syncCustomerActiveOrders).toHaveBeenCalledWith(
        "cust1",
      );
    });
    ...
    it("4. Missing order is regenerated using subscription data (not customer-supplied fields)", async () => {
      ...
      vi.spyOn(orderService, "generateOrdersForSubscription").mockResolvedValue(
        undefined,
      );
    ...
    it("4a. Missing order uses correct deterministic ID format ord_{subId}_{date}_{mealType}", async () => {
      ...
      vi.spyOn(
        orderService,
        "generateOrdersForSubscription",
      ).mockImplementation(async (sub, date, mealTypes) => {
        // Simulate what generateOrdersForSubscription would write — verify the deterministic ID
        capturedOrder = {
          id: `ord_${sub.id}_${date}_${mealTypes[0]}`,
          source: "subscription",
    ...
    it("4c. Wrong customer subscription ownership is denied", async () => {
      ...
      await expect(
        orderService.restoreOrdersForUnskipDay("cust1", "sub1", "2026-08-01", [
          "lunch",
        ]),
    ...
    it("4d. Non-active subscription or invalid dates are rejected during regeneration", async () => {
      ...
      await expect(
        ...
        "Subscription is paused, not active. Cannot regenerate orders.",
      );
