...
type StatusFilter =
  "all" | "scheduled" | "packing" | "packed" | "ready_for_pickup" | "cancelled";
...
function derivePlanLabel(order: {
  source?: string;
  planTier?: string;
}): string {
...
describe("Kitchen Filter Logic — Status Filter", () => {
  ...
  it("skipped orders never appear regardless of status filter", () => {
    const filtersToTest: StatusFilter[] = [
      "all",
      "scheduled",
      "packing",
      "packed",
...
describe("Order Type Badge Derivation from order.source", () => {
  it('subscription order with planTier=basic shows "Basic"', () => {
    expect(derivePlanLabel({ source: "subscription", planTier: "basic" })).toBe(
      "Basic",
    );
  });
  ...
  it('subscription order with planTier=regular shows "Regular"', () => {
    expect(
      derivePlanLabel({ source: "subscription", planTier: "regular" }),
    ).toBe("Regular");
  });
  ...
  it("changing order.source changes badge (not hardcoded)", () => {
    const subLabel = derivePlanLabel({
      source: "subscription",
      planTier: "regular",
    });
