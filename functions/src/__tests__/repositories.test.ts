...
describe("repositories.ts", () => {
  ...
  describe("createRepo", () => {
    ...
    it("create sets data with id", async () => {
      const id = await repositories.userRepository.create(
        { foo: "bar" },
        "my-id",
      );
  ...
  describe("custom repositories", () => {
    it("orderRepository.getCustomerOrdersInRange", async () => {
      const result =
        await repositories.orderRepository.getCustomerOrdersInRange(
          "c1",
          "2026-09-01",
          "2026-09-30",
        );
    ...
    it("holidayRepository.isHoliday returns true when not empty", async () => {
      const result =
        await repositories.holidayRepository.isHoliday("2026-10-02");
    ...
    it("auditRepository.logAction adds log", async () => {
      await repositories.auditRepository.logAction(
        "A",
        "PBY",
        "PROLE",
        "PNAME",
    ...
    it("failureQueueRepository.logFailure adds log", async () => {
      await repositories.failureQueueRepository.logFailure(
        "c1",
        "s1",
        "B",
        "2026",
    ...
    it("notificationService", async () => {
      ...
      await repositories.notificationService.notifyOrderGeneratedCustomer(
        "c",
        "o",
        "m",
        "d",
      ...
      expect(logger.info).toHaveBeenCalledWith(
        "Customer notification: order o generated.",
      );

      await repositories.notificationService.notifyOrderGeneratedDriver(
        "d",
        "o",
        "m",
      );
      expect(logger.info).toHaveBeenCalledWith(
        "Driver notification: order o generated.",
      );

      await repositories.notificationService.notifyDailyOrdersGenerated(
        [],
        "d",
        5,
      );
      expect(logger.info).toHaveBeenCalledWith(
        "Admin notification: 5 daily orders generated for d.",
      );
    });
  });
});
