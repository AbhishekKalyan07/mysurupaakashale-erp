...
export function CustomerOrderHistoryPage() {
  ...
  const {
    ...
    fetchNextPage,
    isFetchingNextPage,
  } = useCustomerOrderHistoryPaginated(firebaseUser?.uid);
          ...
          <div className="space-y-8">
            {groupKeys.map((date) => {
              ...
              const formattedDate = isNaN(dateObj.getTime())
                ? date
                : format(dateObj, "EEEE, MMM do, yyyy");
                        ...
                        >
                          ...
                          <div className="space-y-1.5 text-sm font-sans mt-3 border-t border-rice-200 pt-3">
                            <div className="flex justify-between text-ink-600">
                              ...
                              <span
                                className="font-bold text-ink-900 text-xs truncate max-w-[60%] text-right"
                                title={
                                  order.itemsLabel ||
                                  order.mealName ||
                                  "Standard Meal"
                            ...
                            {order.deliveryResult &&
                              order.status === "failed_delivery" && (
                                <div className="bg-danger/10 p-2 rounded-lg mt-2 text-danger text-xs font-medium">
                                  Failed:{" "}
                                  {order.deliveryResult.reasonCode.replace(
                                    "_",
                                    " ",
                                  )}
                                </div>
