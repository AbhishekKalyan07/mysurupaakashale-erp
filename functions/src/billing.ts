...
export class BillingService {
  ...
  async processSubscriptionEnd(
    ...
    const customerOrders =
      await repoMod.orderRepository.getCustomerOrdersInRange(
    ...
    const payments = await repoMod.paymentRepository.getByCustomerId(
      subscription.customerId,
    );
    ...
    logger.info(
      `[BillingService] ${reason === "cancelled" ? "Cancelled" : "Expired"} subscription ${subscription.id}`,
    );
  }
}
