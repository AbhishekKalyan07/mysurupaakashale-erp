...
describe('orderService', () => {
  ...
  describe('generateDailyOrders', () => {
    it('aggregates orders generated from all meal types', async () => {
      vi.spyOn(orderService, 'generateBreakfastOrders').mockResolvedValue({ generated: 5, failed: 0 });
      vi.spyOn(orderService, 'generateLunchOrders').mockResolvedValue({ generated: 3, failed: 0 });
      vi.spyOn(orderService, 'generateDinnerOrders').mockResolvedValue({ generated: 2, failed: 0 });
      
      const result = await orderService.generateDailyOrders('2026-08-01'); // Saturday

      expect(orderService.generateBreakfastOrders).toHaveBeenCalledWith('2026-08-01');
      expect(orderService.generateLunchOrders).toHaveBeenCalledWith('2026-08-01');
      expect(orderService.generateDinnerOrders).toHaveBeenCalledWith('2026-08-01');
    ...
    it('returns 0 and skips generation if it is Sunday', async () => {
      ...
      expect(result).toEqual({ success: true, message: 'Today is Sunday (Holiday). No orders generated.', ordersGenerated: 0 });
    });

    it('returns success: false and preserves already generated orders if a later meal generation throws a fatal error', async () => {
      vi.spyOn(orderService, 'generateBreakfastOrders').mockResolvedValue({ generated: 5, failed: 0 });
      vi.spyOn(orderService, 'generateLunchOrders').mockRejectedValue(new Error('Database offline'));
      ...
      expect(result).toEqual({ success: false, message: 'Error: Database offline', ordersGenerated: 5 });
    });
  });
  ...
  describe('generateMealOrders (internal logic via mock)', () => {
    it('preserves idempotency and handles missing mealPreferences gracefully', async () => {
      ...
      vi.spyOn(subscriptionRepository, 'list').mockResolvedValue([
        ...
        { id: 'sub1', customerId: 'c1', status: 'active', startDate: '2026-08-01' },
        // Sub 2: Active sub with autoRenew = false (should be generated if it hasn't ended)
        { id: 'sub2', customerId: 'c2', status: 'active', autoRenew: false, startDate: '2026-08-01', endDate: '2026-08-31', mealPreferences: [{ mealType: 'breakfast', selectedOptionId: 'opt1' }] },
        // Sub 3: Active sub that already has an order in todaysOrders (Idempotency check)
        { id: 'sub3', customerId: 'c3', status: 'active', startDate: '2026-08-01', mealPreferences: [{ mealType: 'breakfast', selectedOptionId: 'opt1' }] },
        // Sub 4: Expired sub (status is active, but endDate is in the past). Should be skipped.
        { id: 'sub4', customerId: 'c4', status: 'active', startDate: '2026-07-01', endDate: '2026-08-02', mealPreferences: [{ mealType: 'breakfast', selectedOptionId: 'opt1' }] }
      ] as any);
      ...
      vi.spyOn(orderRepository, 'list').mockResolvedValue([
        { id: 'ord_sub3_2026-08-03_breakfast', subscriptionId: 'sub3', mealType: 'breakfast', status: 'packing' }
      ] as any);
    ...
    it('reruns cleanly and recovers missing orders without global lockout (Idempotency Regression)', async () => {
      ...
      vi.spyOn(subscriptionRepository, 'list').mockResolvedValue([
        { id: 'sub_missing', customerId: 'c1', status: 'active', startDate: '2026-08-01', mealPreferences: [{ mealType: 'breakfast' }] }
      ] as any);
      ...
      const pass1Count = await orderService.generateBreakfastOrders('2026-08-03');
      ...
      batchSet.mockClear();
      vi.spyOn(orderRepository, 'list').mockResolvedValue([
        { id: createdOrderId, subscriptionId: 'sub_missing', mealType: 'breakfast', status: 'scheduled' }
      ] as any);

      const pass2Count = await orderService.generateBreakfastOrders('2026-08-03');
    ...
    it('orchestrates pause-resume and order generation correctly (E2E simulation)', async () => {
      ...
      let subscriptions: any[] = [
        ...
        { id: 'sub_resumed_yesterday', customerId: 'c1', status: 'paused', pauseEndDate: '2026-08-02', startDate: '2026-08-01', mealPreferences: [{ mealType: 'breakfast' }] },
        // Sub 2: Pause ends today. Should stay paused today.
        { id: 'sub_pause_ends_today', customerId: 'c2', status: 'paused', pauseEndDate: '2026-08-03', startDate: '2026-08-01', mealPreferences: [{ mealType: 'breakfast' }] },
        // Sub 3: Pause starts today. Should pause today.
        { id: 'sub_pause_starts_today', customerId: 'c3', status: 'active', pauseStartDate: '2026-08-03', startDate: '2026-08-01', mealPreferences: [{ mealType: 'breakfast' }] },
      ];

      const { subscriptionRepository } = await import('../../firestore/subscriptionRepository');
      
      // Mock list to respect the 'status' filter exactly as the real code does
      vi.spyOn(subscriptionRepository, 'list').mockImplementation(async (...queries: any[]) => {
        let statusFilter: string | null = null;
        queries.forEach(q => {
          if (q.field === 'status' && q.op === '==') statusFilter = q.value;
        });
        if (statusFilter) {
          return subscriptions.filter(s => s.status === statusFilter) as any[];
        }
        return subscriptions as any[];
      });

      // Mock update to mutate our simulated DB
      vi.spyOn(subscriptionRepository, 'update').mockImplementation(async (id: string, updates: any) => {
      ...
      const { automationService } = await import('../../firestore/automationService');
