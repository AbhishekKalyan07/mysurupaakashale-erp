  ...
  "inline-flex items-center gap-1.5 rounded-full font-semibold border transition-colors",
  {
    variants: {
      variant: {
        ...
        success:
          "bg-success-subtle text-success border-success/25 px-3 py-1 text-xs",
        warning:
          "bg-warning-subtle text-warning border-warning/25 px-3 py-1 text-xs",
        danger:
          "bg-danger-subtle text-danger border-danger/25 px-3 py-1 text-xs",
...
export const PremiumBadge = forwardRef<HTMLDivElement, PremiumBadgeProps>(
  ({ className, variant, tone, dot, children, ...props }, ref) => {
    ...
    const dotColor = effectiveVariant
      ? STATUS_DOT_COLORS[effectiveVariant]
      : "bg-text-muted";
    return (
      <div
        ref={ref}
