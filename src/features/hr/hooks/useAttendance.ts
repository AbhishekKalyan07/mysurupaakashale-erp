...
export const queryKeys = {
  attendance: {
    ...
    byDate: (date: string) =>
      [...queryKeys.attendance.base, "date", date] as const,
    byStaff: (staffId: string) =>
      [...queryKeys.attendance.base, "staff", staffId] as const,
    record: (staffId: string, date: string) =>
      [...queryKeys.attendance.base, "record", staffId, date] as const,
  },
};
...
export function useAttendanceByStaff(
  ...
  return useQuery({
    ...
    queryFn: () =>
      attendanceRepository.getAttendanceByStaff(staffId, startDate, endDate),
    enabled: !!staffId,
  });
}
  ...
  return useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: queryKeys.attendance.base,
      });
...
export function useCheckOut() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const totalHours =
        Math.round(((checkOutMinutes - checkInMinutes) / 60) * 10) / 10;
