...
} from "@/shared/components/ui/PremiumBadge";
...
function SubscriptionDetailDialog({
  ...
  const { data: subStats, isLoading: isStatsLoading } = useSubscriptionStats(
    subscription.id,
    subscription.customerId,
  );
  const { data: payment } = usePaymentDetail(
    subscription.latestPaymentId ?? null,
  );
  ...
  const handleConfirm = async () => {
    if (confirmAction === "approve") await approve.mutateAsync(subscription);
      ...
      await reject.mutateAsync({ subscription, reason: reason || undefined });
      ...
      await resume.mutateAsync(subscription);
    onClose();
  };
          ...
          <div className="grid grid-cols-2 gap-3 text-sm font-sans">
            ...
            <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 shadow-sm">
              ...
              <div className="text-text-muted text-xs font-medium capitalize">
                {subscription.planTier} · Qty {subscription.quantity}
              </div>
            ...
            <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 shadow-sm">
              ...
              <div className="font-bold text-primary text-lg font-display">
                ₹{subscription.depositAmount.toLocaleString("en-IN")}
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={async () => {
                    if (
                      !confirm(
                        `Are you sure you want to ${subscription.autoRenew !== false ? "disable" : "enable"} auto-renew for this subscription?`,
                      )
                    )
                      return;
                    try {
                ...
                >
                  <span
                    className={`pointer-events-none inline-block h-3 w-3 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${subscription.autoRenew !== false ? "translate-x-1.5" : "-translate-x-1.5"}`}
                  />
                </button>
              </div>
            </div>
            ...
            {/* Stats */}
            <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 col-span-2 shadow-sm">
              <div className="text-text-muted text-[10px] uppercase tracking-wider font-bold mb-2">
                Delivery Statistics
              </div>
              {isStatsLoading ? (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {subStats && subStats.pausedDates.length > 0 && (
                    <div className="text-xs text-text-muted">
                      Paused on: {subStats.pausedDates.join(", ")}
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 col-span-2 shadow-sm">
              ...
              <div className="flex flex-col gap-3">
                <div>
                  <Badge
                    variant={STATUS_TONE[subscription.status]}
                ...
                {subscription.pauseStartDate && (
                  <div className="text-xs text-text-muted font-medium bg-amber-50 border border-amber-200 p-2 rounded-lg text-amber-900">
                    Pause Scheduled:{" "}
                    <strong>{formatDate(subscription.pauseStartDate)}</strong>
                    {subscription.pauseEndDate && (
                      <span>
                        {" "}
                        to{" "}
                        <strong>{formatDate(subscription.pauseEndDate)}</strong>
                      </span>
                    )}
                  </div>
                )}
                {payment?.screenshotUrl && (
                  <div className="mt-2 border-t border-primary/10 pt-3">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="block w-full max-w-sm rounded-xl overflow-hidden shadow-sm hover:opacity-90 transition-opacity"
                    >
                      <img
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
                ...
                {subscription.deliveryPartnerName && (
                  <Badge variant="info" className="text-[10px] shrink-0">
                    Current: {subscription.deliveryPartnerName}
                  </Badge>
                )}
              </div>
            </div>
              ...
              {!confirmAction ? (
                <>
                  ...
                  {canPause && (
                    <Button
                      variant="secondary"
                      className="w-full gap-2 font-sans font-bold text-amber-600 border-amber-200 hover:bg-amber-50 hover:border-amber-300"
                  ...
                  {canResume && (
                    <Button
                      variant="primary"
                      className="w-full gap-2 font-sans font-bold"
                ...
                <div
                  className={`rounded-xl p-5 ${confirmAction === "approve" || confirmAction === "resume" ? "bg-emerald-50 border border-emerald-200" : confirmAction === "reject" ? "bg-red-50 border border-red-200" : "bg-amber-50 border border-amber-200"} shadow-sm`}
                >
                  <div
                    className={`flex items-center gap-2 mb-3 ${confirmAction === "approve" || confirmAction === "resume" ? "text-emerald-700" : confirmAction === "reject" ? "text-red-700" : "text-amber-700"}`}
                  >
                  ...
                  <p
                    className={`text-sm font-sans mb-5 font-medium ${confirmAction === "approve" || confirmAction === "resume" ? "text-emerald-800" : confirmAction === "reject" ? "text-red-800" : "text-amber-800"}`}
                  >
                    {confirmAction === "approve" &&
                      "This activates the subscription immediately without a linked payment record. Use this for confirmed off-platform payments — otherwise verify their payment from the Payments page instead."}
                    {confirmAction === "reject" &&
                      "This cancels the subscription request and notifies the customer."}
                    {confirmAction === "pause" &&
                      "Deliveries will stop during the selected period."}
                    {confirmAction === "resume" &&
                      "Deliveries will continue as scheduled and any scheduled pauses will be cancelled."}
                  </p>

                  {confirmAction === "pause" && (
                    <div className="flex gap-4 mb-5 text-left bg-white/50 p-4 rounded-lg border border-amber-200/50">
                      <div className="flex-1">
                        <label className="block text-[10px] font-bold text-amber-800 mb-1.5 font-sans uppercase tracking-wider">
                          Start Date
                        </label>
                        <input
                          type="date"
                  ...
                  <div className="flex gap-3">
                    <Button
                      variant="secondary"
                      className="flex-1 font-sans font-bold bg-white/50"
...
function SubscriptionCardView({
  ...
  return (
    <Card className="p-0 overflow-visible relative group shadow-sm border-primary/10 hover:border-primary/30 transition-colors min-w-0">
      {menuOpen && (
        <div
      ...
      >
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-2 items-center mt-1">
            <Badge
              variant={STATUS_TONE[subscription.status]}
              className="text-[10px] uppercase font-bold tracking-wider px-1.5 py-0.5 shadow-sm"
            >
              {subscription.status.replace("_", " ")}
            </Badge>
          </div>
        ...
        <div className="flex items-center gap-2 text-[11px] text-text-muted font-medium mt-0.5 min-w-0">
          ...
          <span className="flex items-center gap-1 truncate min-w-0">
            ...
            <span className="truncate">
              {subscription.customerAddress || "No address"}
            </span>
          </span>
        </div>
        ...
        <div className="flex gap-2 items-center mt-auto overflow-hidden pb-0.5">
          <Badge
            variant="default"
            className="text-[10px] px-1.5 py-0.5 bg-surface-2 text-text font-semibold shrink-0 whitespace-nowrap truncate max-w-[120px]"
          ...
          {subscription.deliveryPartnerName ? (
            <Badge
              variant="info"
              className="text-[10px] px-1.5 py-0.5 shadow-sm text-blue-800 bg-blue-100 border border-blue-200 shrink-0 whitespace-nowrap truncate max-w-[90px]"
            >
              🚚 {subscription.deliveryPartnerName}
            </Badge>
          ) : (
...
export function AdminSubscriptionsPage() {
  const [activeTab, setActiveTab] =
    useState<AdminStatusFilter>("pending_payment");
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useAdminSubscriptions(activeTab);
    ...
    return (
      <div className="space-y-8">
        <PageHeader
          ...
          subtitle="Dashboard / Subscriptions"
        />
        <ErrorState
          title="Could not load subscriptions"
          description="We had trouble loading subscription records."
          onRetry={refetch}
        />
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        userName="Subscriptions"
        subtitle="Manage customer subscription plans, approvals, and status"
      />
        ...
        <EmptyState
          ...
          description={
            search
              ...
              : `No ${activeTab === "all" ? "" : activeTab.replace("_", " ") + " "}subscriptions at this time.`
          }
        />
      ) : (
        <div className="space-y-4">
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      {row.deliveryPartnerName ? (
                        <Badge
                          variant="info"
                          className="text-[10px] px-1.5 py-0.5 shadow-sm text-blue-800 bg-blue-100 border border-blue-200 whitespace-nowrap"
                        >
                          🚚 {row.deliveryPartnerName}
                        </Badge>
                      ) : (
                        <Badge
                          variant="warning"
                          className="text-[10px] px-1.5 py-0.5 shadow-sm whitespace-nowrap"
                        >
                          ⚠ No Driver
                        </Badge>
                      )}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-2 pt-4 border-t border-primary/10 mt-4 text-center sm:text-left">
            <span className="text-xs font-medium text-text-muted">
              Showing <strong className="text-primary">{rows.length}</strong>{" "}
            ...
            {hasNextPage && (
              <Button
                ...
                className="gap-2 font-sans text-xs font-bold text-primary hover:text-gold hover:bg-gold/10"
              >
