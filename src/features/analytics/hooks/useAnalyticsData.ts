  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AnalyticsData> => {
      ...
      summaries.forEach((s) => {
        ...
        Object.entries(s.methodDistribution || {}).forEach(
          ([k, v]) => (methodDist[k] = (methodDist[k] || 0) + v),
        );
        Object.entries(s.deliveryByArea || {}).forEach(
          ([k, v]) => (deliveryByArea[k] = (deliveryByArea[k] || 0) + v),
        );
        Object.entries(s.partnerCount || {}).forEach(
          ([k, v]) => (partnerCount[k] = (partnerCount[k] || 0) + v),
        );
        Object.entries(s.peakHourCount || {}).forEach(
          ([k, v]) => (hourCount[k] = (hourCount[k] || 0) + v),
        );
      });
      ...
      if (isTodayInRange) {
        ...
        todayPayments.forEach((p) => {
          ...
          if (p.status === "verified") {
            ...
            verifiedRev += amt;
            todayRev += amt;
            methodDist[p.paymentMethod] =
              (methodDist[p.paymentMethod] || 0) + amt;
      ...
      const averageOrderValue =
        completedDeliveries > 0 ? totalRev / completedDeliveries : 0;
