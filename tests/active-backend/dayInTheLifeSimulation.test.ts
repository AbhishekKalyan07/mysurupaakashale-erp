...
describe("Day in the Life Operational Simulation (25 Customers)", () => {
  ...
  const mockCustomers: CustomerProfile[] = Array.from(
    { length: 25 },
    (_, i) => ({
      id: `cust_${i + 1}`,
  ...
  it("SIMULATION STEP 2: Full Day Order Generation (Breakfast, Lunch, Dinner for 25 customers)", async () => {
    ...
    vi.spyOn(userRepository, "list").mockImplementation(async (arg?: any) => {
      ...
      if (arg?.value === "kitchen")
        return [{ id: "kitch_1", role: "kitchen", isActive: true }] as any;
      return [] as any;
    });
  ...
  it("SIMULATION STEP 3: Zone & Driver Reassignment Synchronization", async () => {
    ...
    const updateSpy = vi
      .spyOn(clientUserRepository, "update")
      .mockResolvedValue(undefined);
    const syncSpy = vi
      .spyOn(clientOrderService, "syncCustomerActiveOrders")
      .mockResolvedValue();
  ...
  it("SIMULATION STEP 4: Delivery Status Advancement & Driver Lock", async () => {
    ...
    const orderUpdateSpy = vi
      .spyOn(clientOrderRepository, "update")
      .mockResolvedValue(undefined);

    // Driver 1 picks up order
    await deliveryService.markPickedUp(mockOrder.id, "driver_1");
    expect(orderUpdateSpy).toHaveBeenCalledWith(mockOrder.id, {
      status: "picked_up",
    });
    ...
    await expect(
      deliveryService.assignDriver(mockOrder.id, "driver_2", "admin_1"),
  ...
  it("SIMULATION STEP 6: Failure Queue & Audit Logging on Errors", async () => {
    const logFailureSpy = vi
      .spyOn(failureQueueRepository, "logFailure")
      .mockResolvedValue(undefined);
