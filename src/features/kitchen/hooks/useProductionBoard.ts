...
import {
  ...
  type DailyProductionState,
} from "@/shared/services/firestore/dailyProductionRepository";
...
export function useProductionBoard() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = orderRepository.subscribeToDayOrders(
      ...
      (err) =>
        console.error(`[useProductionBoard] orders onSnapshot error:`, err),
    );
    return unsubscribe;
  ...
  const { zoneMap, partnerMap, customerMap, isReferenceLoading } =
    useReferenceData(customerIds);

  // 3. Daily Production State (Realtime)
  const [productionState, setProductionState] =
    useState<DailyProductionState | null>(null);
  useEffect(() => {
    const unsubscribe = dailyProductionRepository.subscribeToState(
      date,
      setProductionState,
    );
    return unsubscribe;
  ...
  const [advancingOrders, setAdvancingOrders] = useState<Set<string>>(
    new Set(),
  );
  ...
  const advanceStatus = async (
    ...
    try {
      ...
      const slaField =
        newStatus === "ready_for_pickup" ? "readyAt" : `${newStatus}At`;
      const updatePayload: any = {
        kitchenStatus: newStatus,
        status: newStatus,
        [slaField]:
          serverTimestamp() as unknown as import("@/shared/types").Timestamp,
        updatedAt:
          serverTimestamp() as unknown as import("@/shared/types").Timestamp,
      };
