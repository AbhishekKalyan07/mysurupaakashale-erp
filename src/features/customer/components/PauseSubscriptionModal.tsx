    ...
    try {
      ...
      toast.success(
        shouldPauseNow
          ...
          : `Pause scheduled starting ${pauseStartDate}`,
      );
      onClose();
    } catch (err: unknown) {
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink-600">
              Select dates to pause your meal deliveries
            </p>
          </div>
        </div>

        <p className="text-ink-600 text-xs font-sans mb-5 leading-relaxed bg-amber-50/60 p-3 rounded-xl border border-amber-200/60">
          Deliveries will be stopped starting from the start date. If you
          specify an end date, deliveries will automatically resume the
          following day.
        </p>
        ...
        <form onSubmit={handlePauseSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-ink-700 mb-1.5 font-sans uppercase tracking-wider flex items-center gap-1.5">
              <Calendar size={14} className="text-emerald-600" /> Pause Start
              Date
            </label>
            <input
              type="date"
          ...
          <div>
            <label className="block text-xs font-bold text-ink-700 mb-1.5 font-sans uppercase tracking-wider flex items-center gap-1.5">
              <Calendar size={14} className="text-emerald-600" /> Pause End Date
              (Optional)
            </label>
            <input
              type="date"
            ...
            <p className="text-[11px] text-ink-500 mt-1 italic">
              Leave empty to pause indefinitely until you choose to resume.
            </p>
          </div>
