...
function formatBillingMonthLong(month: string) {
  try {
    ...
    return new Date(parseInt(year), parseInt(mo) - 1, 1).toLocaleString(
      "en-IN",
      {
        month: "long",
        year: "numeric",
    ...
    try {
      ...
      if (screenshotFile) {
        const { uploadPaymentScreenshot } =
          await import("@/shared/services/storage/uploadPaymentScreenshot");
    ...
    <Card className="border-2 border-amber-400 bg-amber-50/60 p-6 mb-6">
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="text-ink-500 hover:text-ink-700 p-1 shrink-0"
          >
            <XCircle size={18} />
          </button>
        )}
      ...
      <div className="bg-white border border-rice-300 rounded-lg p-4 mb-5 text-sm font-sans">
        <h4 className="font-bold text-ink-800 mb-2 text-xs uppercase tracking-wider">
          Our Payment Details
        </h4>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {form.paymentMethod === "cash" && (
          <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5">
            ...
            <p className="text-xs font-sans text-amber-700">
              For cash payments, hand the amount directly to our staff or
              delivery partner and enter the receipt number below. Our team will
              confirm manually.
            </p>
          </div>
        )}
        ...
        {needsScreenshot && (
          <div>
            ...
            {screenshotPreview ? (
              ...
              >
                <span className="text-sm font-semibold font-sans text-ink-700">
        ...
        {/* Date */}
        <div>
          <label className="block text-xs font-semibold text-ink-700 mb-1.5 font-sans uppercase tracking-wider">
            Date of Payment
          </label>
          <input
            type="date"
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {isBusy ? (
              ...
              <>
                <CheckCircle size={16} className="mr-1" /> Submit Payment
              </>
            )}
          </Button>
        </div>
